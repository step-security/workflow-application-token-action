      ...
      if (installationId) {
        ...
        if (permissionInput) {
          for (let p of permissionInput.split(",")){
            ...
            if (!pLevel) {
              fail(undefined, `Invalid permission format "${p.trim()}" — expected "name:level" (e.g. "issues:read")`);
              return;
            }
        ...
        core.info(`Successfully generated an access token for application, expiring at ${accessToken.expires_at}.`);
